...
The last inequality applies to *all* scalar numbers $x$ and $y$ was not immediately obvious to me. I'm the kind of person that likes numerical proof to internalize these results, and this is where Python comes in. Change the variables `x` and `y` in the code below to see if the inequality holds.
...
Comparing f⋅g with f⋅f and g⋅g

:::

::: {.callout-note}
Feel free to use different functions `f` and `g` in the code
:::

Perhaps you noticed that I formulated the inequality on inner-products, but I'm plotting the functions pointwise. The missing step is the integral, which is evaluated using the `np.trapz()`. As we can confirm below, the inequality holds:
...
$$⟨A, B⟩ = \text{tr}(A^T B)$$
where $\text{tr}$ is the trace operator. Note that this product is different from matrix multiplication $AB ≠ tr(A^T B) = ⟨A, B⟩$. The inequality for matrirces then reads:

$$tr(A^T B) ≤ \frac{1}{2} (tr(A^T A) + tr(B^T B))$$

It's easy to convince yourself that the inequality holds for matrices by writing down the trace as a sum of scalars, because the inequality holds for each scalar in the sum:
